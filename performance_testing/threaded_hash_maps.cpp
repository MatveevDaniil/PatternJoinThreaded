...
template <int P>
int mapreduce_semipattern_search2(
  std::vector<std::string> input,
  const std::string& map_name,
  std::ofstream& ofs,
  size_t true_output_size
) {
  gtl::parallel_flat_hash_map_m<std::string, std::array<idx_vector, P>> map;
  gtl_p_set_idxpair output;
  std::string N = std::to_string(input.size());
  std::string P_str = std::to_string(P);
  tbb::concurrent_vector<std::string> patterns_vector;
  ...
  measure_time(ofs, N + "," + map_name + ",iteration," + P_str, [&]() {
    #pragma omp parallel num_threads(P) 
    {
      ...
      for (size_t i = 0; i < patterns_vector.size(); i++) {
        ...
        std::vector<idx_vector*> loc_idxs_vec;
        loc_idxs_vec.reserve(P);
        for (auto& loc_idxs : map[pattern]) {
          loc_idxs_vec.push_back(&loc_idxs);
        }
        for (size_t I = 0; I < loc_idxs_vec.size(); I++) {
          auto& idxs = *loc_idxs_vec[I];
          for (size_t i = 0; i < idxs.size(); ++i)
...
void mapreduce_semipattern_search2(
  std::vector<std::string> input,
  const std::string& map_name,
  std::ofstream& ofs,
  int P,
  ...
  if (P == 1) {
    mapreduce_semipattern_search2<1>(
      input, map_name, ofs, true_output_size);
  } else if (P == 2) {
    mapreduce_semipattern_search2<2>(
      input, map_name, ofs, true_output_size);
  } else if (P == 4) {
    mapreduce_semipattern_search2<4>(
      input, map_name, ofs, true_output_size);
  } else if (P == 8) {
    mapreduce_semipattern_search2<8>(
      input, map_name, ofs, true_output_size);
  } else if (P == 16) {
    mapreduce_semipattern_search2<16>(
      input, map_name, ofs, true_output_size);
  } else {
    std::cout << "add P=" << P << " to the wrapper" << std::endl;
  }
}
